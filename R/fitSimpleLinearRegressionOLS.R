...
fitSimpleLinearRegressionOLS <- function (x, y) {
	...
	xc <- x - mean(x)  # X mean centered
	yc <- y  - mean(y) # Y mean centered
	...
	# Student coefficient in 95th percentile and with a number of degrees of
	# freedom equal to the number of data points
	t <- qt(0.025, df = length(x) - 2)
